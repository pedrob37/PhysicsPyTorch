...
class nnUNet(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, physics_flag=False, physics_input=None, physics_output=0,
        ...
        self.conv_decode1 = self.expansive_block(120+physics_output, 60, 30)
        self.penultimate_layer = self.final_block(60, 30)
        self.final_layer = self.final_block(30, out_channel)
        
        # Uncertainty
        if self.uncertainty_flag:
            self.unc_conv_decode1 = self.expansive_block(120, 60, 30)
            self.unc_penultimate_layer = self.final_block(60+physics_output, 30)
            self.unc_final_layer = self.final_block(30, out_channel)
    ...
    def forward(self, x, physics=None):
        ...
        if self.physics_flag:
            decode_block2 = self.tiling_and_concat(physics_block, decode_block2)
