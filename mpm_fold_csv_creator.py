...
def sort_dir(dir_name, sub_split=2):
    """
    ...
    return sorted_list, subject_ids, [0] * len(sorted_list)


# Given directory of volumes want to create a csv with 5 folds (training/ valdiation/ inference)
image_directory = '/home/pedro/Project/Simulations/AD_dataset/MPMs/resampled_MPMs/SS_resampled_MPMs'
images, subject_ids, subject_physics = sort_dir(image_directory, sub_split=3)

shuffle_test = False
overall_replications = 119
...
# Fold creation:
def create_csv(dataframe, name='local_physics_csv.csv', folds=True, replications=None):
    if folds:
        dataframe.fold = 0
        ...
        if replications:
            dataframe = dataframe.append([dataframe]*replications, ignore_index=True)
...
create_csv(basic_csv, name='MPM_physics_csv_folds.csv', folds=True, replications=overall_replications)
