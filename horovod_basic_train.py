...
def BespokeDataset(df, transform, patch_size, batch_seed, train=True):
...
# OOD csv
OOD_df = pd.read_csv('/nfs/home/pedro/PhysicsPyTorch/OOD_physics_csv_folds.csv')
...
for fold in range(latest_fold, num_folds):
    ...
    for epoch in range(loaded_epoch, EPOCHS):
        ...
        train_sampler = torch.utils.data.distributed.DistributedSampler(
            patches_training_set, num_replicas=hvd.size(), rank=hvd.rank())
        train_loader = DataLoader(patches_training_set, batch_size=batch_size, shuffle=False, sampler=train_sampler)
        patches_validation_set = BespokeDataset(new_val_df, validation_transform, patch_size, batch_seed=new_seed)
        val_sampler = torch.utils.data.distributed.DistributedSampler(
            patches_validation_set, num_replicas=hvd.size(), rank=hvd.rank())
        val_loader = DataLoader(patches_validation_set, batch_size=val_batch_size, shuffle=False, sampler=val_sampler)
        ...
        if training_mode != 'inference':
            for i, sample in enumerate(train_loader):
                ...
                # Terminal logging
                print(f"iter: {running_iter}, Loss: {loss.item():.4f}, Dice: {pGM_dice:.3f},"
            ...
            if epoch % validation_interval == 0:
                with torch.no_grad():
                    for val_sample in val_loader:
                        ...
                        if physics_flag:
                            ...
                            out2, features_out2 = model(val_images, val_processed_physics)
                        # print(f'Images shape is {images.shape}')
                        else:
                            out, features_out = model(val_images)
                            out2, features_out2 = model(val_images)
                        ...
                        gm_volume2 = gm_out2.view(4, -1).sum(1)
                        names_collector += val_names
                        gm_volumes_collector += gm_volume
                        gm_volumes_collector2 += gm_volume2
                        ...
                        gm_volume_np2 = gm_volume2.cpu().detach().numpy()
                        val_CoV2 = np.std(gm_volume_np2) / np.mean(gm_volume_np2)
                        for i in range(val_batch_size):
                            pGM_dice2 = soft_dice_score(val_labels[i, ...].cpu().detach().numpy(), out2[i, ...].cpu().detach().numpy())
                            metric_collector2 += [pGM_dice2.tolist()]
                            CoV_collector2.append(val_CoV2)
                ...
                if early_stopping.early_stop or epoch == 30:
