...
runai.hpo.init('/nfs/home/pedro/', 'stratification_single')
...
if True:
    ...
    grid=dict(stratification_epsilon=[0.0, 0.1, 1, 10, 50, 100],
...
if not os.path.exists(log_dir):
    os.makedirs(log_dir)
SAVE_PATH = os.path.join(f'/nfs/home/pedro/PhysicsPyTorch/logger/models/{arguments.job_name}_strat_{config["stratification_epsilon"]}')
FIG_PATH = os.path.join(f'/nfs/home/pedro/PhysicsPyTorch/logger/Figures/{arguments.job_name}_strat_{config["stratification_epsilon"]}')
print(FIG_PATH)
        ...
        for epoch in range(loaded_epoch, EPOCHS):
            ...
            if training_mode != 'inference':
                ...
                if epoch % validation_interval == 0:
                    ...
                    print(np.mean(metric_collector), np.mean(CoV_collector), np.std(metric_collector),
                          np.mean(metric_collector) - 4 * np.mean(
                              CoV_collector))
                    runai.hpo.report(epoch=epoch, metrics={'val_dice_std': np.std(metric_collector).tolist(),
                                                           ...
                                                           'val_cov_dice': np.mean(
                                                               metric_collector).tolist() - 4 * np.mean(
...
# HPO logging
runai.hpo.report(epoch=best_epoch, metrics={'val_dice_std': np.std(overall_val_metric).tolist(),
                                            'val_dice': np.mean(overall_val_metric).tolist(),
                                            'val_cov': np.mean(subject_CoVs).tolist(),
                                            'val_cov_dice': np.mean(overall_val_metric).tolist()-4*np.mean(subject_CoVs).tolist()})
