...
print(f'Physics is {physics_flag},  Uncertainty is {uncertainty_flag}')
...
def physics_preprocessing(physics_input, experiment_type):
    if experiment_type == 'MPRAGE':
        ...
        overall_physics = torch.stack((physics_input, TR_expo_params, TE_expo_params, FA_sin_params), dim=1)
...
def corrected_paper_stochastic_loss(logits, sigma, labels, num_passes):
    ...
    for fpass in range(num_passes):
        ...
        # print(f'The sigma mean is {torch.mean(sigma)}, logits mean is {torch.mean(logits)}')
        # print(f'The logits, sigma, ax sizes are: {logits.shape}, {sigma.shape}, {ax.sample(logits_shape).shape}')
        # print(f'A little ax check: {ax.sample(logits_shape)[0, 0, 0, 0, 0]}, {ax.sample(logits_shape)[0, 1, 0, 0, 0]}')
        # else:
...
inference_set = BespokeDataset(inf_df, validation_transform, patch_size=(181, 217, 181), batch_seed=1,
                               queue_length=batch_size)
    ...
    for epoch in range(loaded_epoch, EPOCHS):
        if training_mode != 'inference':
            ...
            for i, sample in enumerate(train_loader):
                ...
                # print(f'The physics are {physics.shape}')
                ...
                if not uncertainty_flag:
                    ...
                    # data_loss = F.binary_cross_entropy(torch.softmax(out + eps, dim=1), labels, reduction='mean')
            ...
            if epoch % validation_interval == 0:
                ...
                current_subject_ids = [int(vn.split('_')[2]) for vn in running_val_names[-1]]
            ...
            def enable_dropout(mymodel):
                for m in mymodel.modules():
                    if m.__class__.__name__.startswith('Dropout'):
                        m.train()
            enable_dropout(model)
            # NO: Don't have this on if test-time dropout samples are desirable!
            # model.eval()
            with torch.no_grad():
                # ponai attempt
                ...
                for inf_sample in inference_set:
                    ...
                    for dropout_sample in range(dropout_samples):
                        if not uncertainty_flag:
                            ...
                            sub = regex.findall(inf_names[0])[0]
                            ...
                            if physics_flag:
                                # print(f'ponai: {input_tensor.shape}, {inf_physics.shape}')
                                inf_processed_physics = physics_preprocessing(inf_physics, physics_experiment_type)
                                val_outputs, unc_val_outputs, sigma_output = sliding_window_inference(
                                    (input_tensor, inf_processed_physics), 160, 1, model, overlap=overlap, mode='gaussian',
                            ...
                            if dropout_level == 0.0:
                                ...
                                if not os.path.exists(inf_path):
                                    os.makedirs(inf_path)
                                save_img(np.squeeze(gm_output_tensor.cpu().numpy()), np.eye(4),
                                         os.path.join(inf_path, f'monai_grid_fold_{fold}_' + os.path.basename(inf_names)))
                                save_img(np.squeeze(unc_output_tensor.cpu().numpy()), np.eye(4),
                                         os.path.join(inf_path, f'unc_monai_grid_fold_{fold}_' + os.path.basename(inf_names)))
                                save_img(np.squeeze(sigma_output.cpu().numpy()), np.eye(4),
                                         os.path.join(inf_path, f'sigma_monai_grid_fold_{fold}_' + os.path.basename(inf_names)))
                            else:
                                inf_subdir = os.path.join(FIG_PATH, f'Inference{dropout_sample}')
                                if not os.path.exists(inf_subdir):
                                    os.makedirs(inf_subdir)
                                save_img(np.squeeze(gm_output_tensor.cpu().numpy()), np.eye(4),
                                         os.path.join(inf_subdir,
                                                      f'monai_grid_fold_{fold}_DO_{dropout_sample}_' + os.path.basename(inf_names)))
                                save_img(np.squeeze(unc_output_tensor.cpu().numpy()), np.eye(4),
                                         os.path.join(inf_subdir,
                                                      f'unc_monai_grid_fold_{fold}_DO_{dropout_sample}_' + os.path.basename(inf_names)))
                                save_img(np.squeeze(sigma_output.cpu().numpy()), np.eye(4),
                                         os.path.join(inf_subdir,
                                                      f'sigma_monai_grid_fold_{fold}_DO_{dropout_sample}_' + os.path.basename(inf_names)))
                                save_img(np.squeeze(gm_output_tensor2.cpu().numpy()), np.eye(4),
                                         os.path.join(inf_subdir,
                                                      f'2monai_grid_fold_{fold}_DO_{dropout_sample}_' + os.path.basename(inf_names)))
                                save_img(np.squeeze(unc_output_tensor2.cpu().numpy()), np.eye(4),
                                         os.path.join(inf_subdir,
                                                      f'2unc_monai_grid_fold_{fold}_DO_{dropout_sample}_' + os.path.basename(inf_names)))
                break
...
overall_subject_ids = [int(vn.split('_')[2]) for vn in overall_val_names]
